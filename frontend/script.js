...
const API = _isLocal
    ? 'http://localhost:5000'
    : (window.location.origin.includes('render.com') ? window.location.origin : 'https://rivaansh-lifesciences.onrender.com');
...
function loadFallbackProducts() {
    _allProducts = [
        { id: 101, name: 'Vitamin C 500mg', price: 249, originalPrice: 299, category: 'vitamins', brand: 'Rivaansh', composition: 'Ascorbic Acid', image: 'https://images.unsplash.com/photo-1505751171710-1f6d0ace5a85?w=400&h=300&fit=crop', badge:'Popular', prescriptionRequired:false },
        { id: 102, name: 'Glucose Powder', price: 179, originalPrice: 219, category: 'nutrition', brand: 'Rivaansh', composition: 'Dextrose', image: 'https://images.unsplash.com/photo-1547489432-cf93fa6c71ee?w=400&h=300&fit=crop', badge:'Trending', prescriptionRequired:false },
        { id: 103, name: 'Paracetamol 500mg', price: 99, originalPrice: null, category: 'medicines', brand: 'Rivaansh', composition: 'Paracetamol', image: 'https://images.unsplash.com/photo-1550572017-ed200f545dec?w=400&h=300&fit=crop', badge:'', prescriptionRequired:false }
    ];
    _filtered = [..._allProducts];
    renderHome();
    renderProductsPage();
...
function cardHTML(p) {
    ...
    const whatsappMsg = encodeURIComponent(`Check out ${p.name} at Rivaansh Lifesciences! ${window.location.href}`);
    ...
    const imgUrl = (p.image && p.image.startsWith('uploads/')) ? `${API}/${p.image}` : (p.image || '');
...
function applyFilters() {
    ...
    if (_currentBrand && _currentBrand !== 'all') {
        list = list.filter(p => p.brand?.toLowerCase() === _currentBrand.toLowerCase());
    }
...
async function createOrderOnServer() {
    ...
    const payload = {
        products: _cart.map(i => ({ productId: i.id, name: i.name, quantity: i.qty, price: i.price })),
...
window.showPage = function(page) {
    ...
    if (pageId === 'adminPanelPage' && (!_user || (_user.role !== 'admin' && !_user.isAdmin))) {
